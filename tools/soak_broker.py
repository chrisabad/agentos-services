"""Phase 1.3 broker sustained-rate soak.

Cycles golden-set requests against `/broker/check` at a steady rate, recording
per-call latency. Reports rolling p50/p99 every minute. Uses `dry_run=True` so
the soak doesn't pollute the real ledger.

Usage:
  AGENTOS_BROKER_URL=http://127.0.0.1:8011 \\
  PAPERCLIP_BOARD_KEY=... \\
  python tools/soak_broker.py [--minutes 30] [--rps 1.0]
"""
...
from broker_golden_set import GOLDEN_SET


def post(base_url: str, token: str, body: dict) -> int:
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=str(Path.home() / ".agentos" / "logs" / "broker-service-soak.csv"),
    )
    ...
    writer.writerow(["ts", "case", "status", "latency_ms"])
    ...
    print(f"soaking broker for {args.minutes}m at ~{args.rps:.1f} rps → {out_path}")
    while time.time() < deadline:
        case = GOLDEN_SET[i % len(GOLDEN_SET)]
        body = dict(case.request)
        body["dry_run"] = True
        i += 1
        t0 = time.perf_counter()
        status = post(args.url, args.token, body)
        ms = (time.perf_counter() - t0) * 1000
        samples_ms.append(ms)
        minute_window.append(ms)
        if status >= 500 or status == 0:
            fives += 1
        writer.writerow([datetime.now(timezone.utc).isoformat(), case.description[:48], status, f"{ms:.1f}"])
        fh.flush()
    ...
    print()
    print(f"=== broker soak summary (n={len(samples_ms)}) ===")
